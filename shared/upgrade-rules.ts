...
export interface ChessLike {
  get(square: Square): { color: Color; type: PieceType } | null | undefined | false;
}

export type Delta = readonly [number, number];

export type PatternResult =
  | { ok: true; noCapture: true }
  | { ok: false; reason: string };

export const FILES: readonly string[] = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'];
export const fileIdx = (sq: Square): number => sq.charCodeAt(0) - 97;
export const rankIdx = (sq: Square): number => parseInt(sq[1] as string, 10) - 1;
export const sqOf = (f: number, r: number): Square => FILES[f] + (r + 1);

// Each entry is [df, dr] — file delta, rank delta. So [1, 0] is "one
// square right, same rank" (a sideways step), not a forward push.
const DELTAS: Record<Exclude<PieceType, 'p'>, readonly Delta[]> = {
...
function pawnDeltas(color: Color): readonly Delta[] {
  const back = color === 'w' ? -1 : 1;
  return [[1, 0], [-1, 0], [0, back]];
}

function deltasFor(type: PieceType, color: Color): readonly Delta[] {
...
export function customMoveTargets(
  type: PieceType,
  from: Square,
  color: Color,
  chess: ChessLike,
