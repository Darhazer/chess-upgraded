...
export default function Lobby({ onJoined }) {
  ...
  const playPublic = () => run(() => lobbyApi.public(socket, name, variant));
  const playBot = () => run(() => lobbyApi.bot(socket, name, variant));
  const createPrivate = () => run(() => lobbyApi.create(socket, name, variant));
  const joinPrivate = () => {
