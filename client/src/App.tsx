...
import { useSocket, useSocketEvent } from './services/socket-context.js';
import { lobbyApi } from './services/lobby-api.js';
import { saveRoom, loadRoom, clearRoom, type SavedRoom } from './services/player-id.js';
...
export default function App() {
  ...
  const [room, setRoom] = useState<SavedRoom | null>(() => loadRoom());
  const [state, setState] = useState<ServerRoomState | null>(null);
  ...
  const [bootstrapped, setBootstrapped] = useState<boolean>(() => !loadRoom());

  useSocketEvent<[ServerRoomState]>('room:state', setState);

  const joinRoom = (next: SavedRoom): void => {
    setRoom(next);
    saveRoom(next);
  };
  ...
  useEffect(() => {
    ...
    void lobbyApi.rejoin(socket, saved.code).then((res) => {
      if (res.ok && res.code && res.color) setRoom({ code: res.code, color: res.color });
  ...
  useEffect(() => {
    ...
    void lobbyApi.join(socket, code).then((res) => {
      if (res.ok && res.code && res.color) joinRoom({ code: res.code, color: res.color });
    });
  }, [connected, socket]);
