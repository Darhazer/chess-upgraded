...
import { computeTapAction, type TapInput } from '../src/features/game/tap-action.js';

const W_PAWN = { type: 'p', color: 'w' as const };
const W_KNIGHT = { type: 'n', color: 'w' as const };
const B_PAWN = { type: 'p', color: 'b' as const };

// Convenience: build the input bag with sensible defaults so each test
// can override just what it cares about.
const ctx = (overrides: Partial<TapInput>): TapInput => ({
  myTurn: true,
  color: 'w',
  selected: null,
  targets: null,
