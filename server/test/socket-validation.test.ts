...
function ready(c: ClientSocket): Promise<void> {
...
type AckResult = { ok: boolean; error?: string; code?: string; [key: string]: unknown };

function emit(c: ClientSocket, event: string, payload: unknown): Promise<AckResult> {
  return c.timeout(1000).emitWithAck(event, payload) as Promise<AckResult>;
}

// Resolve with the next room:state this client receives.
function nextRoomState(c: ClientSocket): Promise<{ players: Array<{ name: string }> }> {
...
after(() => new Promise<void>((resolve) => io.close(() => resolve())));
