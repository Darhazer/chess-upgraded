...
function setup({ fen, kingType } = {}) {
  if (!fen) return new CannibalEngine();
  const cfg = getJSONfromFEN(fen);
  if (kingType) cfg.kingType = { white: 'k', black: 'k', ...kingType };
  return new CannibalEngine(new CannibalBoard(cfg));
}

const tokens = (actions) => new Set(actions.map((a) => `${a.from}${a.to}`));
...
describe('cannibal: king capture-transform', () => {
  ...
  it('a king moving as a knight delivers check', () => {
    ...
    assert.equal(e.status().check, true);
  });
...
describe('cannibal: move notation', () => {
  const lastSan = (e) => e.history[e.history.length - 1].san;
...
describe('cannibal: evaluation', () => {
  ...
  const score = (kingType) => {
    const cfg = getJSONfromFEN('4k3/8/8/8/8/8/8/4K3 w - - 0 1');
    cfg.kingType = { white: 'k', black: 'k', ...kingType };
