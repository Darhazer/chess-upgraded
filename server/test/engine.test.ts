...
import { buildConfig, type MoveAction } from '../src/engine/variant.js';
...
function token(a: { from: string; to: string }): string {
  return `${a.from}${a.to}`;
}
function actionSet(actions: Array<{ from: string; to: string }>): Set<string> {
...
function assertParity(label: string, engine: RulesEngine): void {
  const refActions: ListedAction[] = engine.listActions();
  const forkActions: MoveAction[] = listVariantActions(engine.publicState());
...
describe('engine ↔ RulesEngine action parity', () => {
  const cases: Array<[string, SetupArgs]> = [
  ...
  it('holds at every ply of a short game including an auto-upgrade via capture', () => {
    const e = new RulesEngine();
    const steps: Array<() => { ok: boolean; reason?: string }> = [
      () => e.tryMove('e4'),
      () => e.tryMove('d5'),
...
function refUpgradedAfter(engine: RulesEngine, action: MoveActionInput): Set<string> {
  const e = engine.clone();
  const r = e.applyAction(action);
  assert.equal(r.ok, true, `referee rejected ${JSON.stringify(action)}: ${'reason' in r ? r.reason : ''}`);
  return new Set([...e.upgraded]);
}

function botUpgradedAfter(engine: RulesEngine, from: string, to: string): Set<string> {
...
describe('engine ↔ RulesEngine upgrade-state parity (post-move)', () => {
  const cases: Array<[string, SetupArgs, string, string]> = [
