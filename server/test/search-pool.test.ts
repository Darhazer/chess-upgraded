...
import type { MoveAction } from '../src/engine/variant.js';

const startState = () => new RulesEngine().publicState();

function isLegal(publicState: ReturnType<RulesEngine['publicState']>, action: MoveAction): boolean {
...
describe('SearchPool', () => {
  ...
  it('rejects pending searches when destroyed', async () => {
    ...
    assert.match((b as PromiseRejectedResult).reason.message, /destroyed/);
    // The in-flight search is rejected too, unless the worker happened to answer first.
    if (a!.status === 'rejected') assert.match((a as PromiseRejectedResult).reason.message, /destroyed/);
  });
});
