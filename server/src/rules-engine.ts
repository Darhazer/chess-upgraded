...
import type { Square as ChessJsSquare, Color as ChessJsColor, PieceSymbol, Move, Piece } from 'chess.js';
import {
  FILES,
  fileIdx,
  rankIdx,
...
import type { PieceType } from '../../shared/upgrade-rules.js';
...
export type ActionKind = 'move' | 'custom';
...
export interface TryMoveOk {
  ok: true;
}

export interface TryMoveFail {
  ok: false;
  reason: string;
}

export type TryMoveResult = TryMoveOk | TryMoveFail;

export interface StatusOver extends OverResult {
  over: true;
}
...
export type StatusResult = StatusOver | StatusLive;
...
type InternalMoveOutcome = InternalMoveResult | TryMoveFail;

function noCaptureReason(type: PieceType): string {
...
export class RulesEngine {
  ...
  listActions(color: ChessJsColor = this.turn()): ListedAction[] {
    ...
    for (const sq of this.upgraded) {
      const piece = this.chess.get(sq as ChessJsSquare);
      if (!piece || piece.color !== color) continue;
      const targets = customMoveTargets(piece.type as PieceType, sq, color, this.chess);
  ...
  tryMove(move: string | MoveActionInput | null | undefined): TryMoveResult {
    ...
    if (typeof move === 'object' && move.from) {
      const piece = this.chess.get(move.from as ChessJsSquare);
    ...
    if (
      ...
      const custom = this._tryCustomMove(move as { from: string; to: string });
      if (custom.ok) {
  ...
  _tryStandardMove(move: string | MoveActionInput): InternalMoveOutcome {
    let result: Move;
    try {
      result = this.chess.move(move as string | { from: string; to: string; promotion?: string });
    } catch (err) {
  ...
  _transferUpgradedForStandardMove({ from, to, flags, color, piece, captured }: Move): void {
  ...
  _tryCustomMove(move: { from: string; to: string }): InternalMoveOutcome {
    ...
    const validation = validateCustomPattern(piece.type as PieceType, move.from, move.to, piece.color);
    if (!validation.ok) return { ok: false, reason: validation.reason };

    const dest = this.chess.get(move.to as ChessJsSquare);
    if (validation.noCapture && dest) return { ok: false, reason: noCaptureReason(piece.type as PieceType) };
    ...
    return {
      ...
      ok: true,
      color,
      san: this._customSan(piece.type as PieceType, move.from, move.to, !!dest),
      from: move.from,
      to: move.to,
  ...
  _customCandidate(from: string, to: string): { chess: Chess; upgraded: Set<string> } {
    const piece = this.chess.get(from as ChessJsSquare);
    const dest = this.chess.get(to as ChessJsSquare);
  ...
  _customSan(type: PieceType, from: string, to: string, captured: boolean): string {
    const head = type === 'p' ? '' : type.toUpperCase();
    return `${head}${from}${captured ? 'x' : '-'}${to}*`;
  }

  _buildFenAfterCustom(from: string, to: string, color: ChessJsColor, captured: boolean, flipTurn: boolean): string {
    const board = this.chess.board().map((row) => row.slice()) as (Piece | null)[][];
    ...
    const enPassant = '-';
    let halfmove = parseInt(oldParts[4] ?? '0', 10);
    halfmove = captured || moving.type === 'p' ? 0 : halfmove + 1;
    let fullmove = parseInt(oldParts[5] ?? '1', 10);
  ...
  _afterAction(action: InternalMoveResult): void {
    this.history.push({
      kind: action.kind,
      color: action.color,
      san: action.san,
  ...
  _kingSquareOn(chess: Chess, color: ChessJsColor): string | null {
  ...
  _isInCheck(color: ChessJsColor): boolean {
    return this._isInCheckOn(this.chess, color);
  }

  _isInCheckOn(chess: Chess, color: ChessJsColor): boolean {
    const kingSq = this._kingSquareOn(chess, color);
    if (!kingSq) return false;
    const opp = color === 'w' ? 'b' : 'w';
    return chess.attackers(kingSq as ChessJsSquare, opp).length > 0;
  }

  _hasAnyLegalMove(color: ChessJsColor): boolean {
