...
// Eyeballed, not tuned.
const KING_MOVEMENT_BONUS = { k: 0, q: 8, r: 5, b: 3, n: 3, p: -15 }
...
export default class CannibalBoard extends Board {
    ...
    movementType (piece) {
        if (piece.toLowerCase() === 'k') {
            return this.configuration.kingType[this.getPieceColor(piece)]
        }
    ...
    getKingMoves (piece, location) {
        switch (this.configuration.kingType[this.getPieceColor(piece)]) {
    ...
    getCannibalKingPawnMoves (piece, location) {
        const moves = []
    ...
    playingKingCanCastle () {
        return this.configuration.kingType[this.getPlayingColor()] === 'k'
    }
    ...
    onMoveApplied ({ to, mover, captured, enPassantCapture, movingColor }) {
        const moverIsKing = mover.toLowerCase() === 'k'
        const capturedType = captured
        ...
        if (moverIsKing &&
            this.configuration.kingType[movingColor] === 'p' &&
            (to[1] === '8' || to[1] === '1')) {
            this.configuration.kingType[movingColor] = 'q'
        }
    }
    ...
    calculateScore (playerColor = this.getPlayingColor()) {
        ...
        for (const color of [COLORS.WHITE, COLORS.BLACK]) {
            ...
            bonus += sign * (KING_MOVEMENT_BONUS[this.configuration.kingType[color]] || 0)
        }
        return score + bonus
    }
}
