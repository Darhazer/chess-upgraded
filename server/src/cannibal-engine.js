...
export class CannibalEngine {
  constructor(board = new CannibalBoard()) {
    this.board = board;
  ...
  _action(fromUpper, toUpper) {
    const action = { kind: 'move', from: fromUpper.toLowerCase(), to: toUpper.toLowerCase() };
  ...
  _san(mover, fromUpper, toUpper, captured, moverKingType) {
